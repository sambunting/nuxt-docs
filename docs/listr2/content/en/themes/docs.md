...
_Icon should be a square of at least 512x512_
...
_Image should be at least 640×320px (1280×640px for best display)._
...
- `icon` <badge>v0.7.0+</badge>
  - Type: `String`
  - _Can be used to override the default `type` icon, check out the [icons available](https://github.com/nuxt/content/tree/dev/packages/theme-docs/src/components/global/icons)_
...
<code-group>
  <code-block label="Yarn" active> ```bash yarn add @nuxt/content-theme-docs \``` </code-block>
  <code-block label="NPM"> ```bash npm install @nuxt/content-theme-docs \``` </code-block>
</code-group>
````
