...
- `options.deep`
  ...
  - Default: `false`
  - Version: **>= v1.3.0**
  - _Fetch files from subdirectories_
- `options.text`
  - Type: `Boolean`
  - Default: `false`
  - Version: **>= v1.4.0**
  - _Returns the original markdown content in a `text` variable_
...
const products = await this.$content('products')
  .where({ 'categories.slug': { $contains: 'top' } })
  .fetch()

const products = await this.$content('products')
  .where({ 'categories.slug': { $contains: ['top', 'woman'] } })
  .fetch()
```
...
```js
const [prev, next] = await this.$content('articles').only(['title', 'path']).sortBy('date').where({ isArchived: false }).surround('article-2').fetch()[
  // Returns
  ({
    title: 'Article 1',
    path: 'article-1'
